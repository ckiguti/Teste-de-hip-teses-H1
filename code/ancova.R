#' ---
#' title: "ANCOVA test for `fss`~`dfs`+`scenario`"
...
} else if (packageVersion("rshinystatistics") < "0.0.0.9250") {
  remotes::install_github("geiser/rshinystatistics")
}
...
#' 
## ---- echo=FALSE-------------------------------------------------------------------------------------------------------------------------------
car::Boxplot(`fss` ~ `scenario`, data = dat[["fss"]], id = list(n = Inf))
...
#' ### Textual Report
#' 
#' After controlling the linearity of covariance "dfs", ANCOVA tests with independent between-subjects variables "scenario" (gamified, non.gamified) were performed to determine statistically significant difference on the dependent varibles "fss". For the dependent variable "fss", there was not statistically significant effects.
#' 
#' 
#' 
#' 
